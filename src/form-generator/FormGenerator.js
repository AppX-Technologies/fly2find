...
const FormGenerator = ({
  ...
  const getPreValue = (formValue, field) => {
    ...
    return formValue[field.title || field.id];
  };
        ...
        <Row key={index} id={'row-' + formIndex + '-' + index} className="mb-2">
          {row.columns.map((col, colIndex) => {
            return (
              <Col
                key={colIndex}
                id={'col-' + activeForm + '-' + index + '-' + colIndex}
                xs={col.xs || col.default}
                md={col.md || col.default}
                lg={col.lg || col.default}
                className={`my-2 align-self-end ${col.className || ''}`}
              >
                {col.field.title && (
                  <h6 style={f['compact'] ? { fontSize: 14 } : undefined}>
                    {col.field.title}
