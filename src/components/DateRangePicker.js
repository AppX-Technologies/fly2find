...
const DateRangePicker = ({ label, id, onDateChange, defaultDisabled = false }) => {
      ...
      <Row>
        <Col xs={4}>
          <ListGroup>
            {['Last week', 'This week', 'Last month', 'This month'].map(k => {
              return (
                <ListGroup.Item action onClick={() => setDates(k)} disabled={disabled}>
                  {k}
                </ListGroup.Item>
              );
            })}
        ...
        <Col xs={8} className="align-self-center">
          <Row id="drp-container">
            <Col xs={12}>
              <h6>Start Date</h6>
              <TextInput id="drp-start" disabled={disabled} max={endDate} variant="date" onChange={onStartChange} />
            </Col>
            <Col xs={12} className="mt-2">
              <h6>End Date</h6>
              <TextInput id="drp-end" disabled={disabled} min={startDate} variant="date" onChange={onEndChange} />
            </Col>
          </Row>
        </Col>
      </Row>
